...
public class Trim extends Transformer {
    ...
    public Parser transform(Parser parser) {
        if (parser.get_value(key) == null) {
            logger.debug("Trim transformer failed to transform. Value for Key {} is null", key);
            return parser;
        }
        return parser.set_value(key, ((String) parser.get_value(key)).trim());
    }
